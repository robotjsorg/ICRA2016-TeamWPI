...
def main(imon):
    ...
    if in_collision(start, obstacles, 0):
        ...
        waypoints = planner_waypoint(start, samples, bounds, tsp_targets, tsp_goals, obstacles)
        if len(waypoints) == 0:
            print("Path not found.")
            waypoints = np.asarray([start])
            realizable = False
        else:
            way_to_start = planner_waypoint(tuple(tsp_targets[len(tsp_targets)-1]), samples, bounds, [tsp_targets[0]], [tsp_goals[0]], obstacles)
    ...
    while (not rospy.is_shutdown() ):
        ...
        while (not rospy.is_shutdown() and not lqrc.trial_ended and ((lqrc.error is None) or (la.norm(lqrc.error) > 0.20))): # 0.01
            time.sleep(0.5)
        lqrc.unregister()
        if lqrc.trial_ended:
            return
...
if __name__ == "__main__":
    rospy.init_node("planner_waypoint_lqr", anonymous=True)
